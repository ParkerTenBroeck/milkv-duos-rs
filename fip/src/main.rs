...
mod test{
    ...
    const VT: f64 = per_line(V_SYNC_PULSE) as f64 / 25 as f64;
    const HT: f64 = per_px(H_SYNC_PULSE) as f64 / 25 as f64;
    const VF: f64 = per_line(V_TOTAL) as f64 / 25 as f64;
    const HF: f64 = per_px(H_TOTAL) as f64 / 25 as f64;
    
    #[test]
    fn test(){
        println!("{HT}:{}", HT - 2.3);
        println!("{HF}:{}", HF - 33.36);
        println!("{VT}:{}", VT - 133.56);
        println!("{VF}:{}", VF - 16.68856 * 1000.0);
    }
}
