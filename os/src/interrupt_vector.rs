    ...
    if sync {
        ...
        match code {
            0x7 => {
                ...
                unsafe {
                    // 16 ms
                    timer::add_timercmp(timer::SYS_COUNTER_FREQ_IN_US * 16 * 1000);
                }
...
pub unsafe fn ecall(frame: &mut TrapFrame, mepc: usize) {
    // a7
    let call_id = frame.regs[17 - 1];
    match call_id {
        // read
        63 => {
            let buf =
                core::slice::from_raw_parts_mut(frame.regs[11 - 1] as *mut u8, frame.regs[12 - 1]);
            let mut len = 0;
            while let Some(val) = crate::uart_sys::RX.pop() {
                if buf.len() <= len {
                    break;
                }
                buf[len] = val;
        ...
        64 => {
            let buf =
                core::slice::from_raw_parts(frame.regs[11 - 1] as *mut u8, frame.regs[12 - 1]);
            for byte in buf {
                crate::uart_sys::TX.push(*byte)
            }

            if core::ptr::addr_of_mut!((*mmio::UART0).ier_dlh).read_volatile() & 0b10 == 0 {
                let mut has_space = uart::uart_has_rx_space();
                while has_space {
                    if let Some(b) = crate::uart_sys::TX.pop() {
                        uart::print_c(b);
                        has_space = uart::uart_has_rx_space();
                    } else {
                        break;
                    }
                }
        ...
        _ => {
            println!("Unknown system call: {call_id} addr: {mepc:08x}");
        }
    }
}

pub unsafe fn add_plic_handler(
    int: u32,
    handler: fn(frame: &mut TrapFrame, mcause: usize, mepc: usize, mtval: usize),
) {
    use core::sync::atomic::Ordering;
